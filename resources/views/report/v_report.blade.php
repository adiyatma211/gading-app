...
@section('content')
    <div class="page-heading">
        <h3><i class="bi bi-table me-2"></i>Laporan Transaksi</h3>
    </div>
    <section class="section">
        <div class="card shadow-sm border-primary">
            <div class="card-header bg-primary text-white mb-3">
                <h5 class="card-title mb-0"><i class="bi bi-list-ul me-2"></i>Daftar Transaksi Gading Printing</h5>
            </div>
            <div class="card-body">
                ...
                <div class="card-header">
                    ...
                    <div class="row g-3 mb-3 align-items-end">
                        ...
                        <!-- Filter Minggu -->
                        <div class="col-md-3">
                            <label for="filterMinggu" class="form-label">Pilih Minggu</label>
                            <input type="week" class="form-control" id="filterMinggu">
                        </div>

                        <!-- Tombol Aksi -->
                        <div class="col-md-12 d-flex gap-2 justify-content-end">
                            <button class="btn btn-primary" id="filterBtn"><i class="bi bi-funnel"></i> Filter</button>
                            <button class="btn btn-secondary" id="resetBtn"><i class="bi bi-arrow-counterclockwise"></i>
                                Reset</button>
                ...
                <div class="table-responsive">
                    <table class="table" id="tableRespon">
                        <thead>
                            <tr>
                                <th>No</th>
                                <th>Nama Nasabah</th>
    ...
    <script>
        document.getElementById('exportExcelButton').addEventListener('click', function(e) {
            ...
            // Buat URL dinamis untuk export
            let exportUrl = "{{ route('export.transaksi') }}";
            let params = [];

            // Tambahkan parameter sesuai filter yang ada
            if (startDate && endDate) {
                params.push(`startDate=${encodeURIComponent(startDate)}`);
                params.push(`endDate=${encodeURIComponent(endDate)}`);
            } else if (filterBulan) {
                params.push(`filterBulan=${encodeURIComponent(filterBulan)}`);
            } else if (filterMinggu) {
                params.push(`filterMinggu=${encodeURIComponent(filterMinggu)}`);
            }

            // Gabungkan parameter ke URL
            if (params.length > 0) {
    ...
    <script>
        $(document).ready(function() {
            const table = $('#tableRespon').DataTable({
                processing: true,
                serverSide: true,
                searching: false,
                responsive: true,
                ordering: true,
                ajax: {
                    url: "{{ route('laporan.transaksi.data') }}",
                    type: "POST",
                    data: function(d) {
                        d.startDate = $('#startDate').val();
                        d.endDate = $('#endDate').val();
                        d.filterBulan = $('#filterBulan').val();
                        d.filterMinggu = $('#filterMinggu').val();
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            switch (data) {
                                ...
                                case 'dp':
                                    return '<span class="badge bg-danger text-white">Done Payment</span>';
                                default:
                                    // Jika nilai default, kita juga gunakan badge yang sudah ada
                                    return data;
                            }
                        }
                    }
